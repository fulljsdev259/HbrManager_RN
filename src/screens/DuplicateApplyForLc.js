...
const DuplicateApplyForLc = ({route, navigation}) => {
  ...
  const _renderItem = ({item, index}) => {
    ...
    return (
      <>
        ...
        <GenericTextInput placeholder={list} style={styles.input} />
        {i === item.length - 1 && (
          <GenericButton
            text="Submit"
            ...
            onPress={() =>
              navigation.navigate(
                isToChangePassword ? 'SignIn' : 'AppLoyaltyCard',
              )
            }
          />
        )}
...
export default DuplicateApplyForLc;
